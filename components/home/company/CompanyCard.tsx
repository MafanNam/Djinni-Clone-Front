...
export default function CompanyCard({id, name, image, bio, country, num_employees}) {
  ...
  return (
    <div onClick={() => router.push(`companies/${id}`,{ shallow: true })} className='hover:border-2 border hover:border-[#504ED7] border-gray-200 shadow-md p-5 hover:scale-105 rounded-md transition-[transform] cursor-pointer'>
      <div className='flex items-center gap-2'>
        <div className='w-12 h-12 rounded-full border border-gray-300 shrink-0'>
          <Image src={image} alt={name} width={1000} height={1000} className='w-full h-full rounded-full object-cover' />
        </div>
        <div>
      ...
      <div className='mb-10 mt-6'>
        ...
        <div className='mt-3 text-gray-400 text-sm' dangerouslySetInnerHTML={{ __html: bio.slice(0,30) + '...' }} />
      </div>
